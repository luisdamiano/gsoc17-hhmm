...
stan.model = 'stan/iohmm-mixture.stan'
stan.data = list(
  T = T.length,
  K = K,
  M = M,
...
As in the original work, we set up a IOHMM model to forecast stock prices. We define $K = 4$ hidden states. There are $R = 4$ inputs, namely opening price, closing price, highest price, and the lowest price from the previous time step. The output is stock closing price. The model of the output is a Gaussian mixture with $L = 3$ components per estate, that is

\[
p(x_t | z_{t} = j) = \sum_{l = 1}^{L}{\lambda_l N(x_t | \mu_{l}, \sigma_{l})},
\]

with $0 \le \lambda_l \le 1 \ \forall \ l$ and $\sum_{l=1}^{L}{\lambda_l} = 1$.

we set up a state model involves a multinomial regression whose parameters depend on the previous state taking the value $i$,
