...
parameters {
  ...
  simplex[L] phi_kl[K];             // event probabilities
}
...
transformed parameters {
  ...
  { // Forward algorithm log p(z_t = j | x_{1:t})
    ...
    for (j in 1:K) {
      unalpha_tk[1, j] = log(p_1k[j]) + log(phi_kl[j, x[1]]);
    }

    // for (j in 1:K) {
    //   unalpha_tk[1] = log(p_1k);
    //
    //   if((g[1] == 1 && (j == 1 || j == 4)) || (g[1] == 2 && (j == 2 || j == 3)))
    //     unalpha_tk[1][j] = unalpha_tk[1][j] + log(phi_kl[j, x[1]]);
    // }
    ...
    for (t in 2:T) {
      for (j in 1:K) { // j = current (t)
        for (i in 1:K) { // i = previous (t-1)
          ...
          accumulator[i] = unalpha_tk[t-1, i] + log(phi_kl[j, x[t]]);
    ...
    for (tforward in 0:(T-2)) {
      ...
      for (j in 1:K) { // j = previous (t-1)
        for (i in 1:K) { // i = next (t)
          ...
          accumulator[i] = unbeta_tk[t, i] + log(phi_kl[i, x[t]]);
          if((g[t] == 1 && (i == 1 || i == 4)) || (g[t] == 2 && (i == 2 || i == 3)))
  ...
  { // Viterbi algorithm
    ...
    for (t in 2:T) {
      for (j in 1:K) { // j = current (t)
        ...
        for (i in 1:K) { // i = previous (t-1)
          real logp;
          logp = delta_tk[t-1, i] + log(phi_kl[j, x[t]]);
          // logp = delta_tk[t-1, i] + log(A_ij[i, j]) + log(phi_kl[j, x[t]]);
          if((g[t] == 1 && (j == 1 || j == 4)) || (g[t] == 2 && (j == 2 || j == 3)))
