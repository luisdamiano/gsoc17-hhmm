...
parameters {
  ...
  vector[M] w_k[K];                 // state regressors

  // Continuous observation model
  vector[M] b_k[K];                 // mean regressors
...
transformed parameters {
  ...
  { // Forward algorithm log p(z_t = j | x_{1:t})
    ...
    for(j in 1:K)
      unalpha_tk[1][j] = log(p_1k[j]) + normal_lpdf(x_t[1] | u_t[1]'* b_k[j], s_k[j]);
    ...
    for (t in 2:T) {
      for (j in 1:K) { // j = current (t)
        for (i in 1:K) { // i = previous (t-1)
          ...
          accumulator[i] = unalpha_tk[t-1, i] + log(A_ij[t][i]) + normal_lpdf(x_t[t] | u_t[t]'* b_k[j], s_k[j]);
        }
  ...
  //         accumulator[i] = unbeta_tk[t, i] + log(A_ij[t][i]) + normal_lpdf(x_t[t] | u_t[t]'* b_k[j], s_k[j]);
  //         }
...
generated quantities {
  ...
  // 
  //   for (j in 1:K)
  //     best_logp[1, K] = normal_lpdf(x_t[1] | u_t[1]'* b_k[j], s_k[j]);
  // 
  //   for (t in 2:T) {
  //     for (j in 1:K) {
  ...
  //       for (i in 1:K) {
  //         real logp;
  //         logp = best_logp[t-1, i] + log(A_ij[t][i]) + normal_lpdf(x_t[t] | u_t[t]'* b_k[j], s_k[j]);
