...
transformed parameters {
  ...
  { // Forward algorithm log p(z_t = j | x_{1:t})
    ...
    for (j in 1:K) {
      if(sign[1] == 1) {
        phiadj[1:L] = phi_k[j];
        phiadj[10:18] = rep_vector(0, L);
        // phiadj = append_col(phi_k[j], rep_vector(0, L));
      }

      if(sign[1] == 2) {
        phiadj[10:18] = phi_k[j];
        phiadj[1:L] = rep_vector(0, L);
        // phiadj = append_col(rep_vector(0, L), phi_k[j]);
      }
      unalpha_tk[1][j] = log(p_1k[j]) + log(phiadj[y[1]]);
    }
    ...
    for (t in 2:T) {
      for (j in 1:K) { // j = current (t)
        ...
        for (i in 1:K) { // i = previous (t-1)
          accumulator[i] = unalpha_tk[t-1, i] + log(A_ij[i, j]) + log(phiadj[y[t]]);
        }
