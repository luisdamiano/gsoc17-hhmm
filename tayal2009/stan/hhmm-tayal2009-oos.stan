...
data {
  ...
  int<lower=1> T_oos;               // number of observations (length)
  int<lower=1, upper=L> x_oos[T_oos]; // observations
  int<lower=1, upper=2> sign_oos[T_oos];    // 1 = up, 2 = down
}
...
generated quantities {
  ...
  { // Forward algorithm log p(z_t = j | x_{1:t})
    ...
    for (t in 2:T_oos) {
      ...
      unalpha_tk_oos[t, j] = log_sum_exp(accumulator);
      }
    }
