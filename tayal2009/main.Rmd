...
```{r, include = TRUE, out.width = '\\textwidth', out.height = '0.4 \\textheight', fig.cap = "Features extracted from TSE:G 2007-05-02 10:30:00/2007-05-02 11:30:00."}
...
We present a summary of the estimates in \ref{tab:param-estimates}. We 

\tiny
```{r, include = TRUE}
mat <- summary(stan.fit, pars = c('p_1k', 'A_ij', 'A_ij'),
...
Mixing and convergence to the stationary distribution is acceptable. As an illustration, Figure \ref{} shows the trace plot of some arbitrary parameters. Although re-estating the original Hierarchical Hidden Markov Model into a Hidden Markov Model may increase time and memory complexity, the new model becomes significantly easier to program and convergence. A full list of our results, including convergence statistics such as $\hat{R}$ and the effective sample size, is included in the appendix.
...
Observations are hard assigned to the emission state from the four possible bottom-node) with largest forward probability. By the definition of the hierarchical model, they become naturally linked to one of the two possible top states (bears or bulls).
...
pmat(mat,
     col.names = c('Parameter', 'Mean', 'Mean Std. Error', 'Std. Deviation', '$q_{10\\%}$', '$q_{50\\%}$', '$q_{90\\%}$', 'Eff. sample size', '$\\hat{R}$'),
     row.names = c('$\\pi_{1}$', '$1 - \\pi_{1}$', 
                   '$a_{11}$', '$a_{12}$', '$1 - a_{11} - a_{12}$', 
                   '$a_{22}$', 
                   '$a_{31}$', '$a_{33}$', '$1 - a_{31} - a_{33}$', 
