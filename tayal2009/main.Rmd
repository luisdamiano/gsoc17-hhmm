...
\mat{A} =
  \begin{bmatrix}
    a_{11} & a_{12} & 1 - a_{11} - a_{12} & 0                   \\
    1      & 0      & 0                   & 0                   \\
    a_{31} & 0      & a_{33}              & 1 - a_{31} - a_{33} \\
    0      & 0      & 1                   & 0                   \\
  \end{bmatrix},
\]
...
\[
\mat{\theta} = (\pi_1, a_{11}, a_{12}, a_{31}, a_{33}, b^1_{D_l}, b^2_{U_l}, b^3_{U_l}, b^4_{D_l}),
\]
...
```{r, include = TRUE, out.width = '\\textwidth', out.height = '0.4 \\textheight', fig.cap = "Local extrema detected in TSE:G 2007-05-02 10:30:00/2007-05-02 11:30:00."}
ss <- '2007-05-02 10:30:00/2007-05-02 11:30:00'
opar <- par(TRUE)
par(cex = 0.7, cex.axis = 0.7, cex.label = 0.7)
plot_features(tdata[ss, ], zig[ss, ], which.features = 'extrema')
```

```{r, include = TRUE, out.width = '\\textwidth', out.height = '0.4 \\textheight', fig.cap = "Features extracted from TSE:G 2007-05-02 10:30:00/2007-05-02 11:30:00.", warning = FALSE}
...
pmat(round(mat, 4),
     ...
     align = 'r', scientific = FALSE,
     caption = "\\label{tab:param-estimates}")
```
...
```{r include = TRUE, warning = FALSE, out.width = '\\textwidth', fig.height = 4, fig.cap = "Traceplot of some arbitrarily selected parameters and histograms of diagnostic measures. Mixing, convergence to the stationary distribution and sampling efficiency are acceptable.\\label{fig:diag}"}
...
```{r include = TRUE, warning = FALSE, out.width = '\\textwidth', fig.height = 4, fig.cap = "Distribution of observed features conditional on the estimated hidden regime (top node)."}
...
#              caption = "Summary statistics for the return of the trades assigned to each of the two possible top states. Trade returns were computed as defined in Section \\ref{sec:methodology}. Trade length is computed as the number of ticks involved.")
...
Bear and bull top states have negative and positive mean respectively by construction. As it is also visible in Figure \ref{fig:ret-hist}, a positive skewness coefficient for all states indicates that negative returns tend to overweight positive returns for this specific stock during the five day long in-sample dataset. However, bear markets have a marked skew towards negative returns, becomes more risky in term of extreme events (kurtosis) and trades in bear markets tend to last one tick more than bulls.


```{r include = TRUE, out.width = '\\textwidth', fig.height = 4, fig.cap = "Conditional distribution of trade returns given the estimated top state in TSE:G 2007-05-02 10:30:00/2007-05-02 11:30:00. \\label{fig:ret-hist}"}
plot_topstate_hist(top.ins$ret, top.ins$topstate,
                main.lab = "Return", x.lab = "R")#,
#                cex.names = 0.70, cex.axis = 0.70, cex.lab = 0.70, cex.main = 0.70)
```

We remark that trade returns originated in different stocks or timespans do not share these characteristics. Location, dispersion, symmetry and shape of the returns vary along stocks and day of analysis.
