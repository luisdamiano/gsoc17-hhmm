...
Z_t
  ...
  = \sum_{l}{\psi_t(l) p(z_t = l | \mat{x}_{1:t-1})}.
\]
...
Log domain should be preferred to avoid numerical underflow^[[@bishop2006pattern, @jordan2003introduction] have more detail if we want to enlarge on potential numerical issues].
...
$\alpha_t(j)$ be the filtered belief state at the step $t$ as defined by equation $\ref{eq:filtered-beliefstate}$ and $\beta_t(j)$ be the conditional likelihood of future evidence given that the hidden state at step $t$ is $j$,
...
The Viterbi algorithm is an adaptation of the forwards-backwards algorithm where the forward pass becomes a max-product and the backwards pass relies on a traceback procedure to recover the most probable path. Let $\delta_t(j)$ be the probability of arriving to the state $j$ at step $t$ given the most probable path was taken,
