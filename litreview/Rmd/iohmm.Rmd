...
The first line corresponds to the state model, which consists of a discrete-time, discrete-state hidden states $z_t \in \{1, \dots, K\}$ whose transition depends on the previous state $z_{t-1}$ and the input vector $\mat{u}_{t} \in \RR^M$. Additionally, the observation model is governed by $g(z_{t}, \mat{u}_{t})$, where $\mat{x}_t \in \RR^R$ is the vector of observations, emissions or output. The corresponding joint distribution,
...
can take many forms. In the most frequent parametrization when all the inputs and the outputs are continuous, the state model involves a logistic regression whose parameters depend on the previous hidden states

\[
p(z_t | \mat{x}_{t}, \mat{u}_{t}, z_{t-1} = i) = \text{logit}^{-1}(\mat{w}_i \mat{u}_{t}),
\]

and the observation model is built upon the Gaussian density with parameters depending on the current state

\[
p(\mat{x}_t | \mat{u}_{t}, z_{t} = j) = \mathcal{N}(\mat{x}_t | \mat{b}_j \mat{u}_t, \mat{\Sigma}_j).
\]

IOHMM is fully probabilistic. Hidden states are assumed to follow a multinomial distribution that depends on the input sequence. The transition probabilities $\Psi_t(i, j) = p(z_t = j | z_{t-1} = i, \mat{u}_{1:t})$, which govern the state dynamics, are driven by the control signal as well.
...
<!-- Moreover, the belief states now becomes -->
...
\alpha_t(j) 
  & \triangleq p(z_t = j | \mat{x}_{1:t}, \mat{u}_{1:t}) \\
  & = \psi_t(j) \sum_{i = 1}^{K}{p(z_t = j | z_{t-1} = i, \mat{u}_{t}) p(z_{t-1} = i | \mat{u}_{1:t-1})} \\
  & = \psi_t(j) \sum_{i = 1}^{K}{\Psi_t(i, j) p(z_{t-1} = i | \mat{u}_{1:t-1})} \\
