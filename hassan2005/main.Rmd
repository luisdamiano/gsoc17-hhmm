...
It is very important to note that we are only drawing observations from the fitted model. The forecast methodology proposed by the authors depends on the estimated 
...
For walk forward we've implemented a reduced version of out sampler with only the features needed to compute the forecasts reducing sampling time by a factor of X. The forecasting methodology proposed in the original work requires refitting the model each time a new observation comes. Stan doesn't have a natural way to update a previous fit model (just refit again). Since we run only one chain per stock, we can run many stocks and/or forecast steps at the same time.
...
In a sense, this model could be interpreted as $K \times L$ means that switch depending on previous price levels. We thus expected a highly multinomial, ill-behaved log-posterior that aggravate the frequent combinatorial non-identifiabilities inherent to mixture models. 
