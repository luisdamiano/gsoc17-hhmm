...
wf <- wf_forecast(syms = syms[1, ], K, L, hyperparams,
...
We first recreate the forecast for 30-sep-2004.
...
kable(luv.300904$dataset$u.unscaled[(luv.300904.n - 1):luv.300904.n, ],
      ...
      caption = "Summary of the posterior draws from the distribution of the estimated parameters.")
```

```{r}
for (wf.step in wf[[1]]) {
  points(x = wf.step$forecast
}
```
...
```{r}
fore <- neighbouring_forecasts(x = dataset$x.unscaled, oblik_t = oblik_t,
                      h = 1, threshold = 0.05)
