...
We present an in-depth analysis of one stock, LUV, to assess its strength and weakeness to fit the data and predict out of sample. We then present forecast error measures for all the four involved stocks.
...
We first create a. We attempt to ease convergence by setting up initial values for the component parameters that are approximated with k-means. Although nested k-means is not truthful to the model we replicate, we found that this simplification is useful for a starter point.
...
As suggested by the warning message and the extended guidelines in Stan website, we do not.
...
We extract sampled parameters for later use.
...
### State probability
Comments about the filtered and smoothed. Mention that states are persistents, which makes a lot of sense from the financial point of view.

```{r, warning = FALSE, fig.width = 7, fig.height = 7}
plot_stateprobability(alpha_tk, gamma_tk, 0.8)
```

The jointly most likely path
...
The hidden states capture the variation in the mean, but the observation model is too simplistic. The idea is good tho and it is very well worth exploring IOHMM with better inputs and observation models. By seeing the fit, we don't expect accurate forecasts from this model.

```{r, fig.width = 7, fig.height = 7}
plot_outputfit(dataset$x, hatx_t, K = K)
```

Modifying our implementation to allocate a better observation model would be trivial in most cases.
...
The intervals only represent parameters uncertainty since the method proposed in the original work only allows for point forecasts ^[We should probably make a nice time series plot with intervals].

```{r}
fore <- neighbouring_forecast(x = dataset$x.unscaled, oblik_t = oblik_t,
                      h = 1, threshold = 0.05)
...
* Convergence is hard because the model doesn't capture the dynamics of the data, so the logposterior behaves quite badly.

## All stocks

For walk forward we've implemented a reduced version of out sampler with only the features needed to compute the forecasts. The forecasting methodology proposed in the original work requires refitting the model each time a new observation comes. Stan doesn't have a natural way to update a previous fit model (just refit again). Since we run only one chain per stock, we can run many stocks and/or forecast steps at the same time.
...
In a sense, this model could be interpreted as $K \times L$ means that switch depending on previous price levels. We thus expected a highly multinomial, ill-behaved log-posterior that aggraviate the frequent combinatorial non-identifiabilities inherent to mixture models. 

As our first naive and mostly unrestricted implementation could not sample efficiently from the log-posterior, we decided to add more information in the form of prior belief and additional model structure.

Even after all the improvements, we only arrive at a just about satisfactory yet imperfect answer. We are confident that our implementation works as intended since we tested the model calibration by simulation, some computational issues arise when the sampler is fed with real data however. More specifically, we find that sampling efficiency becomes extremely low for some parameters of the observation model. 

In rasemblance to the folk theorem^[See [The folk theorem of statistical computing
